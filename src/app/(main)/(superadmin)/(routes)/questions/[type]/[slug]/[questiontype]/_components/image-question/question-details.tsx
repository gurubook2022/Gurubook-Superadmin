...
import { DlQuestion, QuestionData, Option as OptionT, } from "../../types";
...
const QuestionDetails = ({ data }: QuestionDetailsProps) => {
  ...
  const saveAudioField = async (
    audioFieldName: "titleAudio" | "subTitleAudio" | "imageTextAudio",
    key: string
  ) => {
  ...
  const saveOptionAudioField = async (optionDataIndex: number, key: string) => {
    const optionLangIndex = getLanguageIndex(
      defaultValues.options?.[optionDataIndex]?.optionData?.map((od: any) => od.language),
      currentLang
    );
    ...
    const patchedOptions = defaultValues.options.map((opt, i) =>
      i === optionDataIndex
        ? {
          ...opt,
          optionData: opt.optionData.map((od: any, j: number) =>
    ...
    try {
      await updateImageQuestion({
        variables: {
          ...
          questionData: defaultValues.questionData,
          chapters: defaultValues.chapters,
          imageUrl: defaultValues.imageUrl,
        },
      });
          ...
          <form
            onSubmit={(e) => {
              ...
              methods.handleSubmit(onSubmit, (errors) => {
                ...
                if ((errors as any).imageUrl) messages.push("Image is required");
              ...
              <div className="md:col-span-3 space-y-4">
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center justify-center gap-2">
                    ...
                    />
                    <AudioUploadButton
                      fileName={buildAudioFileName(data?.questionNumber, "imageText", currentLang)}
                      onUploaded={(key) => saveAudioField("imageTextAudio", key)}
                    />
                  </div>
                </div>
